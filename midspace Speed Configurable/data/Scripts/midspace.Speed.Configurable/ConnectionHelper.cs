namespace midspace.Speed.ConfigurableScript
{
    ...
    public static class ConnectionHelper
    {
        ...
        {
            ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteStart("Start Message Serialization");
            ...
            {
                ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteError("Message cannot Deserialize");
                return;
            }

            ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteStop("End Message Serialization");
            ...
            if (message != null && message.Content != null)
            {
                ...
                {
                    ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteError("Processing message exception. Side: {0}, Exception: {1}", message.Content.Side, e.ToString());
                }
                return;
            }
        }
        ...
        public static int GetFreeByteElementCount(MessageIncomingMessageParts message)
        {
            ...
            //ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteVerbose(string.Format("FinalLength: {0}", finalLength));
        ...
        {
            //ConfigurableSpeedComponentLogic.Instance.ServerLogger.WriteVerbose("SendMessageParts {0} {1} {2}.", byteData.Length, side, receiver);
                ...
                try
                {
                    ...
                    switch (side)
                    {
                        case MessageSide.ClientSide:
                            if (MyAPIGateway.Multiplayer.SendMessageTo(SpeedConsts.ConnectionId, bytes, receiver))
                                byteList.RemoveRange(0, count);
                            else
                                throw new Exception("Failed to send message parts to client.");
                            break;
                        case MessageSide.ServerSide:
                            if (MyAPIGateway.Multiplayer.SendMessageToServer(SpeedConsts.ConnectionId, bytes))
                                byteList.RemoveRange(0, count);
                            else
